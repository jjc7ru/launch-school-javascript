...
class Deck {
  ...
  availableCards() {
    // gets an array of available cards.
    // The card count has to be greater than 0.
    // (ie) ['2', '3', ... ]
...
class Participant {
  ...
  updateScore(card) {
    ...
    // Logic:
    // > If both values in this.score is less than 21 after updateScore()
    //   keep both.
    // > If one of the values are greater than 21, remove that number and
    //   duplicate the first value (ie4)
    // > If both values are greater than 21, keep both (ie5, ie6)
    // (ie1) this.score = [0, 0] --> updateScore(new Card('5'))
    //       --> this.score = [5, 5]
    // (ie2) this.score = [0, 0] --> updateScore(new Card('A'))
    //       --> this.score = [1, 11]
    // (ie3) this.score = [4, 14] --> updateScore(new Card('A'))
    //       --> this.score = [5, 15]
    // (ie4) this.score = [20, 30] --> updateScore(new Card('A'))
    //       --> this.score = [21, 21]
    // (ie5) this.score = [21, 31] --> updateScore(new Card('5'))
    //       --> this.score = [26, 36]
    // (ie6) this.score = [21, 31] --> updateScore(new Card('A'))
    //       --> this.score = [22, 32]
...
class TwentyOneGame {
  ...
  playerBusts() {
    ...
    MessageCenter.displayEmptyLines(1);
    this.messages.displayUpdatedScoreBoard(this.player.getFinalScore(),
  ...
  dealerTurn() {
    ...
    while (true) {
      if (!first) {
        MessageCenter.displayEmptyLines(4);
      }
      MessageCenter.displayEmptyLines(1);
      MessageCenter.displayCards(this.player, this.dealer);
      MessageCenter.displayEmptyLines(1);
      ...
      if ((this.dealer.getScore()[0] >= 17 &&
           this.dealer.getScore()[0] <= 21) ||
          (this.dealer.getScore()[1] >= 17 &&
           this.dealer.getScore()[1] <= 21)) {
  ...
  displayResult() {
    ...
    MessageCenter.displayCards(this.player, this.dealer);
    MessageCenter.displayEmptyLines(1);
    this.messages.displayUpdatedScoreBoard(
      this.player.getFinalScore(), this.dealer.getFinalScore());
    MessageCenter.displayEmptyLines(1);
